...
encrypt_decrypt:     # Encrypts/Decrypts file
    ...
    file_read:        # Reads file to buffer
        ...
        encrypt_loop:
            ...
            lw   $t1, FileBuffer($s0)    # Load character at index in FileBuffer
            lw   $t2, PassBuffer($s1)    # Same as above but in PassBuffer
            ...
            #remove_cont:
            sw   $t0, FileBuffer($s0)    # Store result
            beq  $s0, $s5, encrypt_exit  # If end of buffer reached, exit
            addi $s0, $s0, 4             # Increment fileBuffer index
            addi $s1, $s1, 4             # Increment passBuffer index
            j    encrypt_loop            # Loop
